...
class NEXURA_Ghost_Engine {
    public function scan( $content ) {
        $patterns = [
            'eval\s*\(\s*base64_decode\s*\(' => 'High', 
            'shell_exec\s*\(' => 'Critical', 
            'system\s*\(\s*\$_' => 'Critical', 
            'wp_vcd' => 'High',
            'ndsw===undefined' => 'Critical',
            'ndsx===undefined' => 'Critical',
            'eval\s*\(\s*unescape\s*\(' => 'High',
            'document\.write\s*\(\s*unescape\s*\(' => 'High'
        ];
        foreach ($patterns as $pattern => $risk) {
...
EOT;
    ...
    public function get_cloud_signatures() {
        ...
        // Complies with WordPress.org Guidelines 7 & 9 (no phoning home without consent).
        ...
        $url = $this->api_url . '/signatures?v=' . $version;
        ...
        // Return empty array if API fails, no hardcoded fallbacks
        return [];
    }
