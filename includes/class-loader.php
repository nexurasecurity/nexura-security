...
class Loader {
    ...
    public function run() {
        ...
        if ( version_compare( $db_version, '1.0.12', '<' ) ) {
            ...
            $actual_table = $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $table_scans ) );
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    ...
    private function init_modules() {
        ...
        if ( is_admin() ) {
            $admin->init();
            new Rate_Us_Notice();
            // Show "Rate Us" notice after 14 days
        }
        ...
        if ( file_exists( NEXURA_PLUGIN_DIR . 'includes/class-rest-security-free.php' ) ) {
            require_once NEXURA_PLUGIN_DIR . 'includes/class-rest-security-free.php';
        }
        if ( class_exists( 'Nexura_Security\\REST_Security_Free' ) ) {
            $rest_sec_free = new REST_Security_Free();
            $rest_sec_free->init();
        }
