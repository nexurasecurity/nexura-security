...
class Login_Security {
    public function init() {
        add_action(
            'wp_login',
            [$this, 'enforce_2fa_policy'],
            5,
            2
        );
        add_action(
            'wp_login',
            [$this, 'record_last_login'],
            10,
            2
        );
        ...
        if ( get_option( 'NEXURA_show_last_login', '1' ) === '1' ) {
            ...
            add_filter(
                ...
                [$this, 'populate_last_login_column'],
                10,
                3
            );
        }
    ...
    public function cleanup_recaptcha_logs() {
        ...
        if ( time() - $last_cleanup > DAY_IN_SECONDS ) {
            ...
            $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}NEXURA_recaptcha_logs WHERE timestamp < %s", gmdate( 'Y-m-d H:i:s', strtotime( '-30 days' ) ) ) );
    ...
    public function check_grace_period_enforcement() {
        if ( isset( $_GET['force_setup'] ) && !is_user_logged_in() ) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $token = sanitize_text_field( wp_unslash( $_GET['force_setup'] ) );
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
