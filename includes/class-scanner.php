...
class Scanner {
    ...
    private function get_active_patterns() {
        ...
        if ( file_exists( $file_path ) ) {
            ...
            if ( $content ) {
                ...
                if ( is_array( $raw ) ) {
                    // Validate each pattern before storing â€” skip any that cause preg errors.
    ...
    public function init_scan() {
        ...
        // This is a security scanner â€” it must scan the entire WordPress installation.
        ...
        try {
            ...
            foreach ( $iterator as $file ) {
                if ( $file->isFile() ) {
                    ...
                    // Security: nexura-waf-bootstrap.php is self-generated by Nexura Hardening.
                    // We do NOT blindly whitelist by filename â€” a hacker could name their malware
                    // 'nexura-waf-bootstrap.php' to evade the scanner. Instead, we verify the file's
                    // content signature matches what Nexura actually generates before skipping it.
                    if ( basename( $pathname ) === 'nexura-waf-bootstrap.php' && $this->is_genuine_waf_bootstrap( $pathname ) ) {
                        continue;
                    }
    ...
    private function is_allowed_root_file( $filename ) {
        ...
        // Security: nexura-waf-bootstrap.php is allowed ONLY if its content matches the
    ...
    /**
     * Verifies that a file named nexura-waf-bootstrap.php is genuinely generated
     * by Nexura Security's WAF hardening feature, and not a hacker-placed imposter.
     *
     * A hacker could read the plugin source, learn the filename is whitelisted, and
     * name their malware 'nexura-waf-bootstrap.php' to evade detection. This method
     * prevents that by validating the file's content signature.
     *
     * @param string $full_path Absolute path to the file.
     * @return bool True only if content matches the genuine Nexura WAF bootstrap signature.
     */
    private function is_genuine_waf_bootstrap( $full_path ) {
        if ( ! file_exists( $full_path ) || ! is_readable( $full_path ) ) {
            return false;
        }

        // Read only the first 512 bytes â€” the genuine file is tiny and its signature is at the top.
        // This avoids loading a potentially huge malicious file into memory.
        $handle = @fopen( $full_path, 'r' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
        if ( ! $handle ) {
            return false;
        }
        $header = fread( $handle, 512 ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
        ...
        // The genuine file MUST contain BOTH of these signature lines (written by class-hardening.php).
        // A hacker's malware will not contain both of these exact strings.
        $sig1 = '// Nexura Security WAF Bootstrap';
        $sig2 = '// This file is generated automatically. Do not edit.';
        $sig3 = 'nexura-waf.php';

        return ( strpos( $header, $sig1 ) !== false
            && strpos( $header, $sig2 ) !== false
            && strpos( $header, $sig3 ) !== false );
    }
    ...
    public function process_scan_batch() {
        ...
        if ( empty( $queue ) ) {
            ...
            if ( $issues > 0 ) {
                ...
                if ( time() - $last_email_time > 24 * 3600 ) {
                    ...
                    $message .= '<h2 style="color: #e63946; margin-top: 0;">âš ï¸ Security Alert</h2>';
        ...
        $max_execution_time = 6.0; // 6 seconds per step â€” balanced for speed vs CPU usage
        $current_file = '';
        $recent_files = [];
        $processed_ids = [];
        $new_items = [];
        ...
        if ( $table === 'posts' ) {
            ...
            if ( $results ) {
                foreach ( $results as $row ) {
                    ...
                    // Do NOT run cloud YARA signatures here â€” they match binary patterns and
            ...
            if ( $results ) {
                foreach ( $results as $row ) {
                    ...
                    // Do NOT run cloud YARA signatures here â€” they are designed for binary files
    ...
    public function scan_file( $file_path ) {
        ...
        // 2.6 Advanced JS Heuristics Engine â€” also scan PHP files for injected <script> blocks
    ...
    private function analyze_js_content( $content ) {
        ...
        $js_signatures = [
            ...
            'wscript_shell_exec' => [
                ...
                'description' => 'ActiveXObject WScript.Shell â€” arbitrary command execution trojan.',
                'risk' => 'Critical',
                'confidence' => 98,
            ],
            'filesystem_object' => [
                'pattern' => '/new\s+ActiveXObject\s*\(\s*[\'"]Scripting\.FileSystemObject[\'"]/i',
                'description' => 'ActiveXObject Scripting.FileSystemObject â€” filesystem access trojan.',
                'risk' => 'High',
                'confidence' => 95,
            ],
            'js_shellobj_run' => [
                'pattern' => '/shellObj\.Run\s*\(/i',
                'description' => 'WScript Shell.Run detected â€” stealthy process execution.',
                'risk' => 'Critical',
                'confidence' => 99,
            ],
            'js_suspicious_eval' => [
