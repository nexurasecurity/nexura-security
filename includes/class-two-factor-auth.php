...
class Two_Factor_Auth {
    ...
    public function init() {
        ...
        // Intercept Login
        add_action( 'wp_login', [ $this, 'intercept_login' ], 10, 2 );
    ...
    public function render_profile_notice( $user ) {
        ...
        ?>
        <h2><?php esc_html_e( 'Nexura: Two-Factor Authentication', 'nexura-security' ); ?></h2>
        <p>
            <?php printf( 
                wp_kses( 
    ...
    public function intercept_login( $user_login, $user ) {
        ...
        if ( $is_enabled === '1' ) {
            ...
            set_transient( 'NEXURA_2fa_token_' . $token, $user->ID, 300 ); // 5 minute validity
            ...
            if ( isset( $_REQUEST['redirect_to'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                $redirect_url = add_query_arg( 'redirect_to', urlencode( sanitize_text_field( wp_unslash( $_REQUEST['redirect_to'] ) ) ), $redirect_url ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            }
    ...
    public function handle_2fa_verification_form() {
        ...
        if ( isset( $_POST['NEXURA_2fa_code'] ) ) {
            ...
            $code = preg_replace( '/[^0-9a-zA-Z]/', '', $raw_code ); // Allow letters for recovery codes
            ...
            if ( $is_valid ) {
                ...
                if ( get_option( 'NEXURA_allow_remember_device', '0' ) === '1' && isset( $_POST['NEXURA_remember_device'] ) ) {
                    ...
                    $stored_hashes[] = $hash;
                    if ( count( $stored_hashes ) > 5 ) {
                        array_shift( $stored_hashes );
                    }
                    update_user_meta( $user->ID, 'NEXURA_2fa_remember_hashes', $stored_hashes );
                    setcookie( 'nexura_2fa_remember_' . $user->ID, $hash, time() + 30 * DAY_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN, is_ssl(), true );
                }
        ...
        <form name="NEXURA_2fa_form" id="NEXURA_2fa_form" action="" method="post">
            <?php wp_nonce_field( 'NEXURA_verify_2fa' ); ?>
            <p>
            ...
            <p>
                <label for="NEXURA_remember_device">
                    ...
                    <?php esc_html_e( 'Remember this device for 30 days', 'nexura-security' ); ?>
                </label>
            </p>
            <?php endif; ?>
            <?php if ( isset( $_REQUEST['redirect_to'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
                <input type="hidden" name="redirect_to" value="<?php echo esc_url( sanitize_text_field( wp_unslash( $_REQUEST['redirect_to'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>" />
            <?php endif; ?>
            <p class="submit">
                <input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e( 'Verify', 'nexura-security' ); ?>" />
            </p>
        </form>
        <?php
        login_footer();
    ...
    private function get_current_time() {
        ...
        if ( $offset === false ) {
            ...
            $response = wp_remote_head( 'https://google.com', [ 'timeout' => 3 ] );
    ...
    private function base32_decode( $secret ) {
        ...
        foreach ( str_split( str_replace( '=', '', $secret ) ) as $char ) {
            if ( ! isset( $allowed_values[$char] ) ) continue;
            $binary .= str_pad( decbin( $allowed_values[$char] ), 5, '0', STR_PAD_LEFT );
        }
